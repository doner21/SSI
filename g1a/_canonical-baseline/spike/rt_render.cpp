    ...
    for (uint32_t k = 0; k < N_BLOCKS; ++k)
    {
        ...
        else if (k < K_SWAP)
        {
            ...
            if (k == K_XFADE_START)
            {
                ...
                // Output is discarded — must NOT touch master/aStream/bStream/aXfade/bXfade.
                ...
                for (uint32_t w = 0; w < W_WARM; ++w)
                {
                    ...
                    if (ra != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    st.performers[1].copyOutputFrames(st.outHandle, warmScratch, BLOCK_SIZE);
                }
                ...
                // (c) Initial latch (NOT a scheduled event)
        ...
        else
        {
            ...
            if (k == K_SWAP)
            {
                ...
                float oldBoundary[BLOCK_SIZE];
                {
                    ...
                    if (ra != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    st.performers[1].copyOutputFrames(st.outHandle, oldBoundary, BLOCK_SIZE);
                }

                // (b) reset()+latch-replay B to stateAt(K_SWAP) — NEW post-swap B.
                ...
                // (c) Deliver post-swap events at K_SWAP to the NEW B.
                while (eventIdx < sched.events.size() && sched.events[eventIdx].block == k)
                {
                ...
                float newBoundary[BLOCK_SIZE];
                {
                    ...
                    if (ra != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    st.performers[1].copyOutputFrames(st.outHandle, newBoundary, BLOCK_SIZE);
                }
...
#else
                // ── Variant E baseline — reset()+latch-replay at swap ──
                auto rReset = st.performers[1].reset();
                if (rReset != cmaj::Result::Ok)
                    st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                st.resetCalledOnB = true;
