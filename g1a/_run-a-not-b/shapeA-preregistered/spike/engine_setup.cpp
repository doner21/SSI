...
// engineB (FM2op) build — identical sequence, but captures the DIFFERENT schema
// handles (carrierHz, modIndex, ratio, out). Distinct sessionId.
EngineCtxB buildFM2opEngine(const char* source, int32_t sessionId)
{
    ...
    // Handles captured AFTER load, BEFORE link (SDK-GROUNDING §6.5). The B schema
    // is genuinely different: three input value endpoints with different names.
