...
// ── Boundary micro-fade (PRE-REG BOUNDARY-CLICK §4, Variant G) ───
// Equal-power (constant-power) micro-fade length straddling K_SWAP.
constexpr uint32_t W_BND_SAMPLES   = 256;      // LOCKED (== BLOCK_SIZE)

// ── Variant identity (PRE-REG BOUNDARY-CLICK §3) ─────────────────
#if defined(G1A_VARIANT_F)
constexpr const char* VARIANT_NAME = "F";   // no-reset continuation at swap
#elif defined(G1A_VARIANT_G)
constexpr const char* VARIANT_NAME = "G";   // equal-power boundary micro-fade
#else
constexpr const char* VARIANT_NAME = "E";   // baseline (reset+latch)
#endif
...
constexpr double AMP = 0.25;  // sine amplitude (both A and B use 0.25)

// ── B (FM2op) declared-default inputs (PRE-REG §2.2, §4 B-specific LOCKED) ──
// B's non-carried inputs. Latched ONCE at B's first latch (K_XFADE_START),
// never changed by the schedule. Identical in the live path and every B ref.
constexpr double MOD_INDEX_DEFAULT = 1.0;   // B.modIndex — DECLARED DEFAULT
constexpr double RATIO_DEFAULT     = 1.0;   // B.ratio    — DECLARED DEFAULT
