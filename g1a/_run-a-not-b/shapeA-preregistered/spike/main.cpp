...
// The PRE-DECLARED FALLBACK (PRE-REG A-NOT-B §8) — quoted VERBATIM on FINAL FAIL.
...
int main(int argc, char** argv)
{
    ...
    if (!ctx.ok) { std::cerr << "FATAL: engineA (FreqSine) build failed: " << ctx.error << "\n"; return 3; }
    std::cout << "engineA (FreqSine) built: freqHandle=" << ctx.freqHandle
              << " outHandle=" << ctx.outHandle << "\n";

    // engineB (FM2op) — the STRUCTURALLY DIFFERENT B voice with its own schema.
    EngineCtxB ctxB = buildFM2opEngine(FM2OP_SOURCE, /*sessionId=*/0x5B);
    if (!ctxB.ok) { std::cerr << "FATAL: engineB (FM2op) build/link failed: " << ctxB.error << "\n"; return 3; }
    std::cout << "engineB (FM2op) built: carrierHz=" << ctxB.carrierHzHandle
              << " modIndex=" << ctxB.modIndexHandle << " ratio=" << ctxB.ratioHandle
              << " out=" << ctxB.outHandle << "\n";

    // B-patch provenance: SHA-256 of FM2OP_SOURCE (byte-identical to PRE-REG §2
    // fenced block). The verifier recomputes this over §2 and refuses PASS on any
    // mismatch. b() below records it into the metric set.
    const size_t bSrcBytes = std::strlen(FM2OP_SOURCE);
    const std::string bSourceSha = sha256_hex(FM2OP_SOURCE, bSrcBytes);
    std::cout << "B patch FM2op SHA-256 = " << bSourceSha << " (" << bSrcBytes << " bytes)\n";
    ...
    std::cout << "  REF_Bxfade (engineB, envelope/presence only): " << refBxfade.size() << " samples\n";

    std::vector<float> refBpost = renderRefBpost(ctxB, sched);
    std::cout << "  REF_Bpost (engineB, Variant-F carry-across): " << refBpost.size() << " samples\n";

    // Solo presence refs
    double apreFreq = sched.stateAt(A_PRE_START);
    double kswapFreq = sched.stateAt(K_SWAP);
    std::vector<float> soloApre;
    if (!renderSoloPresence(ctx, apreFreq, soloApre))
    { std::cerr << "FATAL: soloApre render failed\n"; return 4; }
    // renderSolo_B: constant-carrierHz FM2op at kswapFreq — for B-solo click sanity
    // AND Goertzel presence normalization of B's fundamental (B compared to itself).
    std::vector<float> soloBkswap = renderSoloB(ctxB, kswapFreq);
    if (soloBkswap.size() != MEAS_SAMPLES)
    { std::cerr << "FATAL: renderSolo_B render failed\n"; return 4; }
    std::cout << "  Solo presence refs: A@" << apreFreq
              << " Hz (FreqSine), B@" << kswapFreq << " Hz (FM2op)\n";

    // ── Step 4: Event-replay repeats ──
    EventReplayState st;
    st.engineA = ctx.engine;        // FreqSine engine (Performer A)
    st.freqHandle = ctx.freqHandle;
    st.outHandle = ctx.outHandle;
    st.engineB = ctxB.engine;       // FM2op engine (Performer B) — different schema
    st.carrierHzHandle = ctxB.carrierHzHandle;
    st.modIndexHandle  = ctxB.modIndexHandle;
    st.ratioHandle     = ctxB.ratioHandle;
    st.outHandleB      = ctxB.outHandle;

    MetricResults R;
    R.b_source_sha256 = bSourceSha;
    R.b_source_bytes  = (uint64_t)bSrcBytes;
    R.engineB_link_ok = ctxB.ok;
    ...
    for (uint32_t r = 0; r < R_REPEATS; ++r)
    {
        ...
        // Mint a FRESH phase-0 performer A from engineA (FreqSine)
    ...
    // R1 — event-delivery & prepare non-disruption (repeat 0)
    {
        ...
        // PRE-REG §6 R1: xrun==0 & createPerformer_B Ok & engineB_link Ok & windowed
        // events strict '==' & dropped==0 & b_xfade_initial_latch_count==1.
        R.R1_pass = (R.xrun_during_prepare == 0) && R.createPerformer_result_ok
                    && R.engineB_link_ok
                    && R.b_ready_before_xfade
                    && (R.events_delivered_A_prepare == R.events_expected_A_prepare)  // §6 strict equality
    ...
    // R3 — event-response correctness + click-free (repeat 0)
    {
        ...
        // R3c: maxerr_B_xfade RETIRED (PRE-REG §4/§6): undefined for A ≠ B (different
        // waveforms). REF_Bxfade is retained only for the envelope/presence WAV and
        // is NOT part of any bit-exact gate. (No value computed; not gated.)

        // R3d: B[K_SWAP,N) vs REF_Bpost — B compared to itself (Variant-F)
        ...
        // rel_expected_Bpost: B's fundamental (carrierHz) in the post-swap window vs
        // renderSolo_B at the SAME carrierHz — B compared to itself (PRE-REG §6 R3').
        ...
        // R3f: click bound over full output, K_SWAP boundary excluded.
        ...
        // B-solo click sanity: click over renderSolo_B (no boundary in a solo render).
        R.click_max_Bsolo = clickMax(soloBkswap.data(), MEAS_SAMPLES);

        // ── PARAM-CARRY audit (PRE-REG §2.2 / §6 R3') ──
        R.param_carry_modIndex_value = st.modIndexLatchValue;
        R.param_carry_ratio_value    = st.ratioLatchValue;
        R.param_carry_modIndex_count = st.modIndexLatchCount.load();
        R.param_carry_ratio_count    = st.ratioLatchCount.load();
        {
            // Every B carrierHz latch must equal the scheduled freq at that block:
            //  - initial latch (xfadeInitLog) == stateAt(K_XFADE_START)
            //  - each carried event (evLogBPre/evLogBPost) == a scheduled event freq
            //    at that block (carry-across is 1:1 in value + timing).
            bool ok = true;
            if (xfadeInitLog.size() != 1) ok = false;
            else if (std::fabs(xfadeInitLog[0].second - sched.stateAt(K_XFADE_START)) > 0.0) ok = false;
            auto matchesSchedule = [&](const std::pair<uint32_t,double>& e) {
                for (const auto& se : sched.events)
                    if (se.block == e.first && std::fabs(se.freq - e.second) == 0.0) return true;
                return false;
            };
            for (const auto& e : evLogBPre)  if (!matchesSchedule(e)) ok = false;
            for (const auto& e : evLogBPost) if (!matchesSchedule(e)) ok = false;
            R.param_carry_carrierHz_ok = ok;
        }
        R.param_carry_audit_ok = R.param_carry_carrierHz_ok
            && (R.param_carry_modIndex_count == 1) && (R.param_carry_modIndex_value == 1.0)
            && (R.param_carry_ratio_count == 1)    && (R.param_carry_ratio_value == 1.0);

        // PRE-REG §6 R3' (A ≠ B, frozen): maxerr_reload_A==0 & mix_err<=1e-6 &
        // equal-power monotone(a²+b²≈1) & maxerr_reload_B==0 & click_max<=0.05 &
        // click_max_Bsolo<=0.05 & rel_Apre>=0.40 & rel_Bpost>=0.40 &
        // param_carry_audit_ok.  maxerr_B_xfade is RETIRED (not gated).
    ...
    // ── 6. Write proof artifacts ──
    {
        ...
        writeWavFloat(ART_DIR + "Solo_A_" + std::to_string((int)apreFreq) + ".wav",
            soloApre.data(), MEAS_SAMPLES, SAMPLE_RATE);
        // B-solo proof (renderSolo_B): the click-sanity + Goertzel normalization render.
        writeWavFloat(ART_DIR + "B_solo.wav", soloBkswap.data(), MEAS_SAMPLES, SAMPLE_RATE);
        writeWavFloat(ART_DIR + "Solo_B_" + std::to_string((int)kswapFreq) + ".wav",
    ...
    std::cout << "R3'(event+equalpower xfade): " << (R.R3_pass?"PASS":"FAIL")
              << "  maxerr_A=" << R.maxerr_reload_A
              << " mix_err=" << R.mix_err
              << " maxerr_B=" << R.maxerr_reload_B
              << " click=" << R.click_max
              << " click_Bsolo=" << R.click_max_Bsolo
              << " rel_Apre=" << R.rel_expected_Apre
    ...
    std::cout << "B patch FM2op SHA-256=" << R.b_source_sha256
