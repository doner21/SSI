...
{
    // EQUAL-POWER: master xfade == a(k)·A + b(k)·B with per-block coefficients,
    // where k = K_XFADE_START + (m / BLOCK_SIZE).
    double mx = 0.0;
    for (uint64_t m = 0; m < M; ++m)
    {
        const uint32_t k = K_XFADE_START + (uint32_t)(m / BLOCK_SIZE);
        const double ca = xfadeCoeffA(k);
        const double cb = xfadeCoeffB(k);
        const double ref = ca * (double)a[m] + cb * (double)b[m];
...
// Equal-power coefficient audit over the W_XFADE crossfade blocks:
//   a(k) monotone DOWN, b(k) monotone UP, a(k)²+b(k)² ≈ 1 (constant power).
bool alphaMonotone(uint64_t /*M*/)
{
    double prevA = 2.0, prevB = -1.0;
    for (uint32_t i = 0; i < W_XFADE; ++i)
    {
        const uint32_t k = K_XFADE_START + i;
        const double a = xfadeCoeffA(k);
        const double b = xfadeCoeffB(k);
        if (a > prevA) return false;   // a non-increasing
        if (b < prevB) return false;   // b non-decreasing
        if (std::fabs(a * a + b * b - 1.0) > 1e-9) return false;  // constant power
        prevA = a; prevB = b;
    }
    // Last crossfade block must be fully B (a≈0, b≈1) just before the swap.
    if (std::fabs(xfadeCoeffA(K_SWAP - 1)) > 1e-9) return false;
    if (std::fabs(xfadeCoeffB(K_SWAP - 1) - 1.0) > 1e-9) return false;
    return true;
}
...
// Whole-buffer click / slew bound (no boundary exclusion) — renderSolo_B sanity.
