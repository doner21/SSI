...
bool writeTraceLog(const std::string& path, const MetricResults& r)
{
    ...
    f << "── A ≠ B provenance (two-engine + FM2op B patch) ──\n";
    f << "  b_source_sha256 = " << r.b_source_sha256 << " (" << r.b_source_bytes
      << " bytes; verifier recomputes vs PRE-REG §2 fenced block)\n";
    f << "  engineB_link = " << (r.engineB_link_ok ? "Ok" : "Fail") << " (gate: Ok)\n\n";

    f << "── R1 event-delivery & prepare non-disruption ──\n";
    f << "  xrun_during_prepare = " << r.xrun_during_prepare << " (gate: ==0)\n";
    f << "  createPerformer_B = " << (r.createPerformer_result_ok ? "Ok" : "Fail") << " (gate: Ok)\n";
    ...
    f << "  mix_err = " << r.mix_err << " (gate: <= 1e-6, EQUAL-POWER coeffs)\n";
    f << "  alpha_monotone = " << (r.alpha_monotone ? "true" : "false")
      << " (a v, b ^, a^2+b^2~1)\n";
    f << "  maxerr_B_xfade = RETIRED (A ≠ B: undefined for different waveforms)\n";
    f << "  maxerr_reload_B = " << r.maxerr_reload_B << " (gate: ==0, B-vs-B)\n";
    f << "  click_max = " << r.click_max << " (gate: <= " << S_CLICK << ", K_SWAP boundary excluded)\n";
    f << "  click_max_Bsolo = " << r.click_max_Bsolo << " (gate: <= " << S_CLICK << ", renderSolo_B)\n";
    f << "  rel_expected_Bpost = " << r.rel_expected_Bpost << " (gate: >= 0.40, B vs renderSolo_B)\n";
    f << "  rel_expected_Apre = " << r.rel_expected_Apre << " (gate: >= 0.40)\n";
    f << "  param_carry_audit_ok = " << (r.param_carry_audit_ok ? "true" : "false")
      << " (carrierHz==scheduled freq; modIndex=" << r.param_carry_modIndex_value
      << " x" << r.param_carry_modIndex_count << "; ratio=" << r.param_carry_ratio_value
      << " x" << r.param_carry_ratio_count << ")\n";
...
bool writeMetricsJson(const std::string& path, const MetricResults& r)
{
    ...
    o << "  \"run_params\": { \"fs\":48000, \"block\":256, \"n_blocks\":10000, "
      ...
      << "\"mod_index\":" << MOD_INDEX_DEFAULT << ", \"ratio\":" << RATIO_DEFAULT << ", "
      << "\"crossfade_type\":\"equal-power\", "
      << "\"w_warm\":" << W_WARM << ", \"w_bnd_samples\":" << W_BND_SAMPLES << " },\n";

    o << "  \"b_patch\": { \"name\":\"FM2op\", "
      << "\"b_source_sha256\":\"" << r.b_source_sha256 << "\", "
      << "\"b_source_bytes\":" << r.b_source_bytes << ", "
      << "\"sha_convention\":\"PRE-REG §2 fenced cmajor block, UTF-8, LF, trailing newline included\", "
      << "\"engineB_link\":\"" << (r.engineB_link_ok ? "Ok" : "Fail") << "\", "
      << "\"schema\":[\"carrierHz\",\"modIndex\",\"ratio\",\"out\"] },\n";

    o << "  \"R1\": { \"xrun_during_prepare\":" << r.xrun_during_prepare
      << ", \"createPerformer_B\":\"" << (r.createPerformer_result_ok ? "Ok" : "Fail") << "\""
      << ", \"engineB_link\":\"" << (r.engineB_link_ok ? "Ok" : "Fail") << "\""
      ...
      << ", \"pass\":" << b(r.R1_pass)
      << ", \"threshold\":\"xrun==0 & createPerformer_B Ok & engineB_link Ok & done<4000 & delivered==expected & dropped==0 & b_xfade_initial_latch==1\" },\n";
    ...
    o << "  \"R2\": { \"asan_violations\":" << r.asan_violations
      ...
      << ", \"pass\":" << b(r.R2_pass)
      << ", \"threshold\":\"[R2' frozen] asan==0 & crt==0 & result==0 & swap_block==4064 & swap_count==1 & xrun_at_swap==0 & released_post_swap (reset_called_on_B NOT mandated)\" },\n";

    o << "  \"R3\": { \"maxerr_reload_A\":" << r.maxerr_reload_A
      << ", \"mix_err\":" << r.mix_err
      << ", \"alpha_monotone\":" << b(r.alpha_monotone)
      << ", \"maxerr_B_xfade\":\"RETIRED\""
      ...
      << ", \"click_max_Bsolo\":" << r.click_max_Bsolo
      << ", \"click_boundary_excluded\":" << b(r.click_boundary_excluded)
      << ", \"param_carry_modIndex_value\":" << r.param_carry_modIndex_value
      << ", \"param_carry_ratio_value\":" << r.param_carry_ratio_value
      << ", \"param_carry_modIndex_count\":" << r.param_carry_modIndex_count
      << ", \"param_carry_ratio_count\":" << r.param_carry_ratio_count
      << ", \"param_carry_carrierHz_ok\":" << b(r.param_carry_carrierHz_ok)
      << ", \"param_carry_audit_ok\":" << b(r.param_carry_audit_ok)
#if defined(G1A_VARIANT_G)
      << ", \"maxerr_reload_B_region\":\"[K_SWAP+W_BND_SAMPLES, N)\""
#else
      << ", \"maxerr_reload_B_region\":\"[K_SWAP, N)\""
#endif
      << ", \"pass\":" << b(r.R3_pass)
      << ", \"threshold\":\"[R3' A≠B] maxerr_A==0 & mix_err<=1e-6 & monotone(a^2+b^2~1) & maxerr_B==0 & click<=0.05 & click_Bsolo<=0.05 & rel_Apre>=0.40 & rel_Bpost>=0.40 & param_carry_audit_ok (maxerr_Bxfade RETIRED)\" },\n";
    ...
    o << ", \"repro_ok\":" << b(r.repro_ok)
      ...
      << ", \"pass\":" << b(r.R4_pass)
      << ", \"threshold\":\"[R4 frozen] repro_ok & blocks==10000 & dropped==0 & dup==0 & event_audit_all_match (applied_A==pre_swap & applied_B_pre==xfade & applied_B_post==post_swap)\" },\n";

    o << "  \"p50_ns\":" << r.p50_ns << ", \"p99_ns\":" << r.p99_ns
      << ", \"max_ns\":" << r.max_ns << ",\n";
    o << "  \"asan_clean\":" << b(r.asan_clean) << ",\n";
    o << "  \"proof_complete\":" << b(r.proof_complete) << ",\n";
    o << "  \"b_source_sha256\":\"" << r.b_source_sha256 << "\",\n";
