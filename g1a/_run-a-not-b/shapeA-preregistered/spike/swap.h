...
struct EventReplayState
{
    ...
    // ── TWO-ENGINE references (PRE-REG §2.1) ──
    // A ≠ B: A and B have different endpoint schemas, so they can no longer share
    // one engine/program. engineA drives Performer A (performers[0]); engineB is
    // touched SOLO by the worker to create Performer B (performers[1]). Each engine
    // is accessed by exactly one thread at a time; the RT thread never touches
    // either engine after createPerformer. Both engines are kept alive here.
    cmaj::Engine engineA;            // FreqSine engine (Performer A)
    uint32_t     freqHandle { 0 };   // A: "freq"      input value handle
    uint32_t     outHandle  { 0 };   // A: "out"       output stream handle

    cmaj::Engine engineB;            // FM2op engine (Performer B) — DIFFERENT schema
    uint32_t     carrierHzHandle { 0 };  // B: "carrierHz" input value — PARAM-CARRY target
    uint32_t     modIndexHandle  { 0 };  // B: "modIndex"  input value — default 1.0
    uint32_t     ratioHandle     { 0 };  // B: "ratio"     input value — default 1.0
    uint32_t     outHandleB      { 0 };  // B: "out"       output stream handle
    ...
    std::atomic<uint64_t> eventsAppliedAPrepare { 0 };  // A events during B-prepare window (windowed R1)
    std::atomic<uint64_t> bXfadeInitialLatchCount { 0 };  // B carrierHz initial latch — should be 1
    std::atomic<uint64_t> latchReplayCount    { 0 };  // should be 1 (0 in Variant F)
    std::atomic<uint64_t> bWarmupCount        { 0 };  // warm-up audit (Variant E, un-gated)

    // ── PARAM-CARRY audit (PRE-REG §2.2) — B's declared-default inputs ──
    // modIndex and ratio are latched ONCE at B's first latch (K_XFADE_START) to
    // their declared defaults (1.0) and never changed by the schedule.
    std::atomic<uint64_t> modIndexLatchCount { 0 };  // should be 1
    std::atomic<uint64_t> ratioLatchCount    { 0 };  // should be 1
    double                modIndexLatchValue { 0.0 };  // should be 1.0
    double                ratioLatchValue    { 0.0 };  // should be 1.0
    ...
    void resetForRepeat(uint32_t r, bool enablePerturb, uint64_t seed)
    {
        ...
        eventsAppliedAPrepare.store(0, std::memory_order_relaxed);
        bXfadeInitialLatchCount.store(0, std::memory_order_relaxed);
        bWarmupCount.store(0, std::memory_order_relaxed);
        latchReplayCount.store(0, std::memory_order_relaxed);
        modIndexLatchCount.store(0, std::memory_order_relaxed);
        ratioLatchCount.store(0, std::memory_order_relaxed);
        modIndexLatchValue = 0.0;
        ratioLatchValue = 0.0;
