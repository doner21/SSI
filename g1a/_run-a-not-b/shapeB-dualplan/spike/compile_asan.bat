@echo off
REM G1A EVENT-REPLAY — ASan build (HARNESS-SPEC §7). Adds /fsanitize=address,
REM /Zi /DEBUG for better ASan reporting (SMOKE-TEST §7), /DG1A_ASAN tag.
REM Requires clang_rt.asan_dynamic-x86_64.dll next to the .exe at runtime.
REM Captures output to build_asan.log for certification audit.
call "C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Auxiliary\Build\vcvars64.bat" > nul 2>&1
cl.exe /std:c++17 /EHsc /O2 /Zi /utf-8 /DCMAJOR_DLL=1 /DG1A_ASAN /DG1A_VARIANT_F /fsanitize=address ^
  /I"C:\Users\doner\SSI\g1a\_run\cmajor-headers\include" ^
  /Fe:g1a_event_replay_host_asan.exe ^
  main.cpp engine_setup.cpp event_schedule.cpp bg_prepare.cpp rt_render.cpp crossfade.cpp swap.cpp reference.cpp perturb.cpp metrics.cpp proof.cpp ^
  /link /DEBUG ^
  > build_asan.log 2>&1
exit /b %ERRORLEVEL%
