...
bool writeTraceLog(const std::string& path, const MetricResults& r)
{
    ...
    f << "  reset_called_on_B = " << (r.reset_called_on_B ? "true" : "false") << " (gate: NOT mandated for Variant F)\n";
    f << "  latch_replay_called = " << (r.latch_replay_called ? "true" : "false") << " (gate: NOT mandated for Variant F)\n";
    ...
    f << "  latch_replay_count = " << r.latch_replay_count << " (Variant F: no reset/latch-replay; not gated)\n";
...
bool writeMetricsJson(const std::string& path, const MetricResults& r)
{
    ...
    o << "  \"run_params\": { \"fs\":48000, \"block\":256, \"n_blocks\":10000, "
      ...
      << "\"mod_index\":" << r.mod_index << ", \"ratio\":" << r.ratio << " },\n";
    ...
    o << "  \"R1\": { \"xrun_during_prepare\":" << r.xrun_during_prepare
      ...
      << ", \"pass\":" << b(r.R1_pass)
      << ", \"threshold\":\"xrun==0 & engineB_link==Ok & createPerformer==Ok & done<4000 & delivered==expected & dropped==0 & b_xfade_initial_latch==1\" },\n";
    ...
    o << "  \"R3\": { \"maxerr_reload_A\":" << r.maxerr_reload_A
      ...
      << ", \"pass\":" << b(r.R3_pass)
      << ", \"threshold\":\"maxerr_A==0 & mix_err<=1e-6 & monotone(a^2+b^2~=1) & maxerr_B==0 & click<=0.05 & click_Bsolo<=0.05 & rel_Apre>=0.40 & rel_Bpost>=0.40 vs renderSolo_B & param_carry_audit_ok (maxerr_B_xfade UNGATED)\" },\n";
    ...
    o << ", \"repro_ok\":" << b(r.repro_ok)
      ...
      << ", \"pass\":" << b(r.R4_pass)
      << ", \"threshold\":\"[R4 frozen] repro_ok & blocks==10000 & dropped==0 & dup==0 & event_audit_all_match\" },\n";
    ...
    o << "  \"xfade_convention\":\"" << r.xfade_convention << "\",\n";
    o << "  \"b_patch_sha\":\"" << r.b_patch_sha << "\",\n";
    o << "  \"b_patch_sha_convention\":\"" << r.b_patch_sha_convention << "\",\n";
    o << "  \"xfade_coeffs\": [";
    for (uint32_t i = 0; i < W_XFADE; ++i)
    {
        const double a = equalPowerAlpha(i);
        const double b = equalPowerBeta(i);
        o << "[" << a << ", " << b << "]";
        if (i + 1 < W_XFADE) o << ", ";
    }
    o << "],\n";
    o << "  \"per_repeat\": [";
    for (size_t i = 0; i < r.per_repeat.size(); ++i)
    {
        o << "{\"hash_full\":\"" << r.per_repeat[i].hash_full
          << "\", \"hash_A\":\"" << r.per_repeat[i].hash_A
          << "\", \"hash_B\":\"" << r.per_repeat[i].hash_B
          << "\", \"hash_xfade\":\"" << r.per_repeat[i].hash_xfade << "\"}";
        if (i + 1 < r.per_repeat.size()) o << ", ";
    }
    o << "],\n";
