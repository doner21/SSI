                      ...
                      bool timed)
{
    ...
    for (uint32_t k = 0; k < N_BLOCKS; ++k)
    {
        ...
        if (k < K_XFADE_START)
        {
            ...
            {
                deliverEvent(st, st.performers[0], st.aFreqHandle,
            ...
            advanceOne(st, st.performers[0], st.aOutHandle, outBlock);
        ...
        else if (k < K_SWAP)
        {
            ...
            // Initial latch sets ALL THREE B inputs: carrierHz, modIndex, ratio.
            if (k == K_XFADE_START)
            {
                ...
                for (uint32_t w = 0; w < W_WARM; ++w)
                {
                    ...
                    if (ra != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    st.performers[1].copyOutputFrames(st.bOutHandle, warmScratch, BLOCK_SIZE);
                }
                st.bWarmupCount.fetch_add(1, std::memory_order_relaxed);
                ...
                // (c) Initial latch: ALL THREE B inputs (carrierHz, modIndex, ratio).
                // Counts as ONE initial-latch episode (bXfadeInitialLatchCount).
                double initFreq = sched.stateAt(k);
                {
                    auto r1 = st.performers[1].setInputValue(st.bCarrierHzHandle, (float)initFreq, 0);
                    if (r1 != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    auto r2 = st.performers[1].setInputValue(st.bModIndexHandle, (float)MOD_INDEX, 0);
                    if (r2 != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                    auto r3 = st.performers[1].setInputValue(st.bRatioHandle, (float)RATIO, 0);
                    if (r3 != cmaj::Result::Ok)
                        st.resultViolations.fetch_add(1, std::memory_order_relaxed);
                }
                st.bXfadeInitialLatchCount.fetch_add(1, std::memory_order_relaxed);
                st.paramCarryModIndexSets.fetch_add(1, std::memory_order_relaxed);
                st.paramCarryRatioSets.fetch_add(1, std::memory_order_relaxed);
                if (!st.paramCarryLog.empty())
                    st.paramCarryLog.push_back({k, initFreq, MOD_INDEX, RATIO});
                if (!st.xfadeInitialLatchLog.empty())
                    st.xfadeInitialLatchLog.push_back({k, initFreq});
            }

            // Deliver scheduled events to BOTH A and B (param carry-across)
            while (eventIdx < sched.events.size() && sched.events[eventIdx].block == k)
            {
                double freq = sched.events[eventIdx].freq;
                // Deliver to A (freq handle)
                deliverEvent(st, st.performers[0], st.aFreqHandle, freq, k,
                             st.eventLogA.empty() ? nullptr : &st.eventLogA);
                st.eventsAppliedA.fetch_add(1, std::memory_order_relaxed);

                // Carry to B (carrierHz handle): 1:1 value+timing
                deliverEvent(st, st.performers[1], st.bCarrierHzHandle, freq, k,
            ...
            advanceOne(st, st.performers[0], st.aOutHandle, tmpA);
            advanceOne(st, st.performers[1], st.bOutHandle, tmpB);
            ...
            // Host crossfade mix (per-block equal-power)
            mixCrossfadeBlock(tmpA, tmpB, outBlock, static_cast<uint32_t>(k - K_XFADE_START));
        }
        ...
        {
            // Variant F handles block K_SWAP entirely inline (no-reset continuation);
            // Variant G is not compiled when G1A_VARIANT_F is defined.
            ...
            if (k == K_SWAP)
            {
                ...
                // ── Variant F — no-reset continuation (PRE-REG §3) ──
                // Do NOT reset() or re-latch. The already-warmed, phase-correct B
                // (its naturally-advanced crossfade-end state) IS the swap state.
                // carrierHz was already latched at K_XFADE_START + carried events.
                st.resetCalledOnB    = false;
                st.latchReplayCalled = false;
                // modIndex/ratio remain at declared defaults 1.0.
#endif
            }

            if (!boundaryHandled)
            {
                // Deliver scheduled events to B (post-swap): carried on bCarrierHzHandle
                while (eventIdx < sched.events.size() && sched.events[eventIdx].block == k)
                {
                    deliverEvent(st, st.performers[1], st.bCarrierHzHandle,
                                 sched.events[eventIdx].freq, k,
                ...
                advanceOne(st, st.performers[1], st.bOutHandle, outBlock);
