...
//   Each engine is accessed by exactly ONE thread at a time.
//   Main thread finishes all engine ops before worker touches engineB.
//   Worker calls createPerformer(B) on engineB solo.
//   RT thread never touches either engine.
// Single-thread-per-engine preserved because engineA and engineB are distinct objects.
#pragma once
...
struct EventReplayState
{
    ...
    // ── TWO-ENGINE split (PRE-REG §2.1) ──
    // engineA = FreqSine (handles freq, out) — session 0x5A
    // engineB = FM2op (handles carrierHz, modIndex, ratio, out) — session 0x5B
    // The engines are KEPT ALIVE here. Main thread sets them once.
    // Worker calls engineB.createPerformer() solo. RT thread never touches either engine.
    cmaj::Engine engineA;
    cmaj::Engine engineB;
    // A handles (FreqSine)
    uint32_t aFreqHandle { 0 };   // "freq" input value handle
    uint32_t aOutHandle  { 0 };   // "out" output stream handle
    // B handles (FM2op)
    uint32_t bCarrierHzHandle { 0 }; // "carrierHz" input value handle
    uint32_t bModIndexHandle  { 0 }; // "modIndex" input value handle
    uint32_t bRatioHandle     { 0 }; // "ratio" input value handle
    uint32_t bOutHandle       { 0 }; // "out" output stream handle
    ...
    // Param-carry audit (PRE-REG §2.2): log every B input set
    std::vector<ParamCarryEntry> paramCarryLog;
    std::atomic<uint64_t> paramCarryModIndexSets { 0 };
    std::atomic<uint64_t> paramCarryRatioSets    { 0 };
    ...
    void resetForRepeat(uint32_t r, bool enablePerturb, uint64_t seed)
    {
        ...
        paramCarryLog.clear();
        paramCarryModIndexSets.store(0, std::memory_order_relaxed);
        paramCarryRatioSets.store(0, std::memory_order_relaxed);
    }
};
