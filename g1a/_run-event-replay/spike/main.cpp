...
"PRE-DECLARED FALLBACK — G1A event-replay spike (committed before any result exists):\n"
"\n"
"On FINAL FAIL of the G1A event-replay spike, the verdict is: `event-replay not feasible on this host`.\n"
"\n"
"(1) Investment-freeze means: immediately HALT all further investment in the live-crossfade\n"
"    event-replay bridge. No additional engineering is spent on atomic-swap reset-and-replay\n"
"    of event-reactive Cmajor performers. The later event-replay passes (phase-continuous,\n"
"    MIDI, transport-sync, multi-stream) are NOT started. The event-replay question is\n"
"    escalated to Donald for a platform decision.\n"
"\n"
"(2) The immediate fallback for event-replay across the reload boundary is: after the\n"
"    atomic-index swap at K_SWAP, the host calls performer B's reset() and then\n"
"    re-STATES the current latched frequency every block (idempotently) instead of\n"
"    relying on the latch-replay strategy. This trades the bit-exact-vs-reference\n"
"    verification for a structurally-safe event-replay that cannot lose events but\n"
"    cannot verify replay fidelity.\n";
...
int main(int argc, char** argv)
{
    ...
    // R1 — event-delivery & prepare non-disruption (repeat 0)
    {
        ...
        R.events_expected_A_prepare = sched.eventsInRange(K_PREPARE_START,
                                                          R.k_prepare_done > K_PREPARE_START ?
                                                              R.k_prepare_done : K_PREPARE_START);
        R.events_delivered_A_prepare = st.eventsAppliedA.load();
        ...
        R.R1_pass = (R.xrun_during_prepare == 0) && R.createPerformer_result_ok
                    ...
                    && (R.events_delivered_A_prepare >= R.events_expected_A_prepare)  // at least the expected count
    ...
    // R3 — event-response correctness + click-free (repeat 0)
    {
        ...
        R.R3_pass = (R.maxerr_reload_A <= 1e-6) &&  // allow tiny rounding diffs as "zero"
                    (R.mix_err <= EPS_XFADE) && R.alpha_monotone
                    && (R.maxerr_B_xfade <= 1e-6)
                    && (R.maxerr_reload_B <= 1e-6)
                    && (R.rel_expected_Bpost >= REL_MIN)
                    && (R.rel_expected_Apre >= REL_MIN);
        // Click check: if > S_CLICK, fail (but boundary-aware is lenient enough)
        // (R3f is additional; the bit-exact checks are the primary signal)
    }
