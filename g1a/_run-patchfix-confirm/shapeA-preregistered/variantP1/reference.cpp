...
// Fresh performer, phase 0. Initial latch at K_XFADE_START, then W_XFADE blocks.
...
std::vector<float> renderRefBpost(EngineCtx& ctx, const EventSchedule& sched)
{
    ...
    // Variant F oracle: initial normalization latch at K_XFADE_START (matching the
    // RT warm-up reset+latch state), advance through the crossfade and CONTINUE
    // across K_SWAP with NO reset/re-latch. Bit-exact target over [K_SWAP, N).
    perf.setInputValue(ctx.freqHandle, (float)sched.stateAt(K_XFADE_START), 0);
    ...
    for (uint32_t k = K_XFADE_START; k < N_BLOCKS; ++k)
    {
        ...
        perf.copyOutputFrames(ctx.outHandle, blk, BLOCK_SIZE);
        if (k >= K_SWAP)
        {
