...
// The PRE-DECLARED FALLBACK (PRE-REG §7 HYBRID) — quoted VERBATIM on FINAL FAIL.
static const char* FALLBACK_VERBATIM =
"PRE-DECLARED FALLBACK — G1A swap-boundary click (PRE-REGISTRATION §7):\n"
"\n"
"If the swap-boundary click cannot be brought <= 0.05 while preserving bit-exact\n"
"post-swap (maxerr_reload_B==0) and crossfade phase (maxerr_B_xfade==0), adopt the\n"
"HYBRID bridge: warm-up-normalized crossfade for the regenerate transition plus\n"
"per-block idempotent frequency re-statement across the K_SWAP boundary (re-send\n"
"current input values every block at/after the swap; state converges within one\n"
"block period) in place of the reset+latch-replay swap. If the crossfade phase\n"
"itself regresses, fall all the way back to full per-block idempotent frequency\n"
...
int main(int argc, char** argv)
{
    ...
    // R1 — event-delivery & prepare non-disruption (repeat 0)
    {
        ...
        const uint32_t k_prepare_done_capped = R.k_prepare_done > K_PREPARE_START ?
            R.k_prepare_done : K_PREPARE_START;
        R.events_expected_A_prepare = sched.eventsInRange(K_PREPARE_START, k_prepare_done_capped);

        // Windowed delivered: count eventLogA entries with block in [K_PREPARE_START, k_prepare_done]
        {
            uint64_t delivered = 0;
            for (const auto& e : evLogA)
                if (e.first >= K_PREPARE_START && e.first <= k_prepare_done_capped)
                    ++delivered;
            R.events_delivered_A_prepare = delivered;
        }
        ...
        R.R1_pass = (R.xrun_during_prepare == 0) && R.createPerformer_result_ok
                    ...
                    && (R.events_delivered_A_prepare == R.events_expected_A_prepare)  // strict ==
    ...
    // R3 — event-response correctness + click-free (repeat 0)
    {
        ...
        // Extended click analysis over master0
        {
            ClickAnalysis ca = clickAnalyze(master0.data(), TOTAL_SAMPLES, boundarySample, K_SWAP);
        ...
        // Engine-intrinsic determination: analyse REF_A (no-swap oracle)
        {
            uint64_t refASamples = (uint64_t)K_SWAP * BLOCK_SIZE;
            uint64_t refABoundary = refASamples;  // no swap → no exclusion, but keep same exclusion
            ClickAnalysis caRefA = clickAnalyze(refA.data(), refASamples, refABoundary, K_SWAP);
            R.refA_click_max = caRefA.click_max;
            R.refA_argmax_sample = caRefA.argmax_sample;
            R.refA_argmax_block  = caRefA.argmax_block;
            R.refA_slew_median   = caRefA.slew_median;
            R.refA_slew_p95      = caRefA.slew_p95;
            R.refA_slew_p99      = caRefA.slew_p99;
            R.refA_slew_mean     = caRefA.slew_mean;

            // Does REF_A reproduce the identical click at the identical sample?
            // Use R.argmax_sample (saved from master0 analysis) since 'ca' is out of scope.
            R.ref_a_reproduces_click =
                (caRefA.argmax_sample == R.argmax_sample) &&
                (std::fabs(caRefA.click_max - R.click_max) < 1e-9) &&
                (R.argmax_sample < boundarySample);  // argmax in [0, K_SWAP)

            if (R.ref_a_reproduces_click)
                R.engine_intrinsic_verdict = "ENGINE_INTRINSIC";
            else
                R.engine_intrinsic_verdict = "SWAP_BOUNDARY";
        }

        // Variant-aware maxerr_reload_B scoping
#if defined(G1A_VARIANT_F)
        // Variant F: full post-swap range
        R.maxerr_reload_B_scope = "full";
        R.maxerr_bnd_window = 0.0;
#elif defined(G1A_VARIANT_G)
        // Variant G: post micro-fade window (per pre-reg §3)
        // maxerr_reload_B over [K_SWAP + W_BND_SAMPLES, N)
        // maxerr_bnd_window reported separately for transparency
        {
            uint64_t bndSkipSamples = (uint64_t)W_BND_SAMPLES;
            uint64_t postMicroSamples = (uint64_t)(N_BLOCKS - K_SWAP) * BLOCK_SIZE - bndSkipSamples;
            if (postMicroSamples > 0)
            {
                const float* masterBndStart = master0.data() + boundarySample + bndSkipSamples;
                const float* refBndStart = refBpost.data() + bndSkipSamples;
                R.maxerr_reload_B = maxAbsDiff(masterBndStart, refBndStart, postMicroSamples);

                // also compute in-window residual
                const float* masterBndWin = master0.data() + boundarySample;
                R.maxerr_bnd_window = maxAbsDiff(masterBndWin, refBpost.data(), bndSkipSamples);
            }
            R.maxerr_reload_B_scope = "post_bnd_window (per pre-reg §3, W_BND_SAMPLES=256)";
        }
#else
        // Base: full range
        R.maxerr_reload_B_scope = "full";
        ...
        // Strict gate (per spec §5 gate table)
    ...
    std::cout << "R3 (event+crossfade):       " << (R.R3_pass?"PASS":"FAIL")
              ...
              << " (scope: " << R.maxerr_reload_B_scope << ")"
              << " rel_Bpost=" << R.rel_expected_Bpost
              << " click=" << R.click_max << "\n";
    std::cout << "  click argmax: sample=" << R.argmax_sample
              << " block=" << R.argmax_block
              << " offset=" << R.argmax_offset
              << " prev=" << R.argmax_prev
              << " cur=" << R.argmax_cur
              << " is_K_SWAP=" << (R.argmax_is_K_SWAP?"Y":"N") << "\n";
    std::cout << "  slew: median=" << R.slew_median
              << " p95=" << R.slew_p95
              << " p99=" << R.slew_p99
              << " mean=" << R.slew_mean << "\n";
    std::cout << "  REF_A (no-swap): click=" << R.refA_click_max
              << " argmax_sample=" << R.refA_argmax_sample
              << " argmax_block=" << R.refA_argmax_block
              << " reproduces=" << (R.ref_a_reproduces_click?"YES":"NO")
              << " verdict=" << R.engine_intrinsic_verdict << "\n";
    std::cout << "  REF_A slew: median=" << R.refA_slew_median
              << " p95=" << R.refA_slew_p95
              << " p99=" << R.refA_slew_p99 << "\n";
