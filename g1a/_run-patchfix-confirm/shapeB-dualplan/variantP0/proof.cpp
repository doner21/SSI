...
bool writeTraceLog(const std::string& path, const MetricResults& r)
{
    ...
    f << "  W_WARM=" << W_WARM << ", W_BND_SAMPLES=" << W_BND_SAMPLES << "\n";
    ...
    f << "  reset_called_on_B = " << (r.reset_called_on_B ? "true" : "false") << " (informational, not gated in R2')\n";
    f << "  latch_replay_called = " << (r.latch_replay_called ? "true" : "false") << " (informational, not gated in R2')\n";
    f << "  xrun_at_swap = " << r.xrun_at_swap << " (ns, gate: ==0)\n";
    ...
    f << "  maxerr_reload_B_scope = " << r.maxerr_reload_B_scope << "\n";
    if (r.maxerr_bnd_window > 0.0 || r.maxerr_reload_B_scope.find("bnd_window") != std::string::npos)
        f << "  maxerr_bnd_window = " << r.maxerr_bnd_window << " (in-window residual, informational)\n";
    ...
    f << "  click_argmax: sample=" << r.argmax_sample << " block=" << r.argmax_block
      << " offset=" << r.argmax_offset << " prev=" << r.argmax_prev << " cur=" << r.argmax_cur
      << " is_K_SWAP=" << (r.argmax_is_K_SWAP ? "true" : "false") << "\n";
    f << "  per_sample_slew: median=" << r.slew_median << " p95=" << r.slew_p95
      << " p99=" << r.slew_p99 << " mean=" << r.slew_mean << "\n";
    f << "  REF_A (no-swap oracle): click=" << r.refA_click_max
      << " argmax_sample=" << r.refA_argmax_sample << " argmax_block=" << r.refA_argmax_block << "\n";
    f << "  REF_A slew: median=" << r.refA_slew_median << " p95=" << r.refA_slew_p95
      << " p99=" << r.refA_slew_p99 << " mean=" << r.refA_slew_mean << "\n";
    f << "  ref_a_reproduces_click = " << (r.ref_a_reproduces_click ? "true" : "false")
      << " (engine_intrinsic_verdict: " << r.engine_intrinsic_verdict << ")\n";
    ...
    f << "  latch_replay_count = " << r.latch_replay_count << " (informational, not gated in R4)\n";
...
bool writeMetricsJson(const std::string& path, const MetricResults& r)
{
    ...
    o << "  \"run_params\": { \"fs\":48000, \"block\":256, \"n_blocks\":10000, "
         ...
         "\"w_warm\":" << W_WARM << ", \"w_bnd_samples\":" << W_BND_SAMPLES << " },\n";
    ...
    o << "  \"R1\": { \"xrun_during_prepare\":" << r.xrun_during_prepare
      ...
      << ", \"pass\":" << b(r.R1_pass)
      << ", \"threshold\":\"xrun==0 & result Ok & done<4000 & delivered==expected (strict) & dropped==0 & b_xfade_initial_latch==1\" },\n";
    ...
    o << "  \"R2\": { \"asan_violations\":" << r.asan_violations
      ...
      << ", \"pass\":" << b(r.R2_pass)
      << ", \"threshold\":\"asan==0 & crt==0 & result==0 & swap_block==4064 & swap_count==1 & xrun_at_swap==0 & released_post_crossfade (reset_called_on_B and latch_replay not gated)\" },\n";
    ...
    o << "  \"R3\": { \"maxerr_reload_A\":" << r.maxerr_reload_A
      ...
      << ", \"maxerr_reload_B_scope\":\"" << r.maxerr_reload_B_scope << "\""
      << ", \"maxerr_bnd_window\":" << r.maxerr_bnd_window
      ...
      << ", \"click_argmax_sample\":" << r.argmax_sample
      << ", \"click_argmax_block\":" << r.argmax_block
      << ", \"click_argmax_offset\":" << r.argmax_offset
      << ", \"click_argmax_prev\":" << r.argmax_prev
      << ", \"click_argmax_cur\":" << r.argmax_cur
      << ", \"click_argmax_is_K_SWAP\":" << b(r.argmax_is_K_SWAP)
      << ", \"slew_median\":" << r.slew_median
      << ", \"slew_p95\":" << r.slew_p95
      << ", \"slew_p99\":" << r.slew_p99
      << ", \"slew_mean\":" << r.slew_mean
      << ", \"refA_click_max\":" << r.refA_click_max
      << ", \"refA_argmax_sample\":" << r.refA_argmax_sample
      << ", \"refA_argmax_block\":" << r.refA_argmax_block
      << ", \"refA_slew_median\":" << r.refA_slew_median
      << ", \"refA_slew_p95\":" << r.refA_slew_p95
      << ", \"refA_slew_p99\":" << r.refA_slew_p99
      << ", \"refA_slew_mean\":" << r.refA_slew_mean
      << ", \"ref_a_reproduces_click\":" << b(r.ref_a_reproduces_click)
      << ", \"engine_intrinsic_verdict\":\"" << r.engine_intrinsic_verdict << "\""
      << ", \"pass\":" << b(r.R3_pass)
      << ", \"threshold\":\"maxerr_A==0 (strict) & mix_err<=1e-6 & monotone & maxerr_Bxfade==0 (strict) & maxerr_B==0 (strict) & rel_Bpost>=0.40 & rel_Apre>=0.40 & click<=0.05\" },\n";
    ...
    o << ", \"repro_ok\":" << b(r.repro_ok)
      ...
      << ", \"pass\":" << b(r.R4_pass)
      << ", \"threshold\":\"repro_ok & blocks==10000 & dropped==0 & dup==0 & swap_block==4064 & applied_A==scheduled_pre_swap & applied_B_pre==scheduled_xfade & applied_B_post==scheduled_post_swap & b_xfade_initial_latch==1 (latch_replay_count not gated)\" },\n";
