...
ClickAnalysis clickAnalyze(const float* x, uint64_t M, uint64_t boundarySample, uint32_t kSwap)
{
    ...
    // Collect all per-sample slews (except boundary pair)
    std::vector<double> slews;
    slews.reserve((size_t)(M > 0 ? M - 1 : 0));

    for (uint64_t k = 1; k < M; ++k)
    {
        if (k == boundarySample) continue;  // skip K_SWAP boundary pair
        double d = std::fabs((double)x[k] - (double)x[k - 1]);
        slews.push_back(d);
        if (d > ca.click_max)
        {
            ca.click_max = d;
            ca.argmax_sample = k;
            ca.argmax_prev = x[k - 1];
            ca.argmax_cur = x[k];
        }
    }

    ca.argmax_block = ca.argmax_sample / BLOCK_SIZE;
    ca.argmax_offset = (uint32_t)(ca.argmax_sample % BLOCK_SIZE);
    ca.is_K_SWAP = (ca.argmax_sample == boundarySample) || (ca.argmax_sample + 1 == boundarySample);

    // Percentiles from collected slews
    if (!slews.empty())
    {
        std::sort(slews.begin(), slews.end());
        size_t n = slews.size();
        ca.slew_median = slews[(size_t)(0.50 * (double)(n - 1))];
        ca.slew_p95    = slews[(size_t)(0.95 * (double)(n - 1))];
        ca.slew_p99    = slews[(size_t)(0.99 * (double)(n - 1))];
        double sum = 0.0;
        for (double v : slews) sum += v;
        ca.slew_mean = sum / (double)n;
    }
