...
// ref-counted COM wrapper — NEVER held in std::atomic<cmaj::Performer> (§2). The
// COM ref-count itself is internally atomic, making the safe release race-free.
#pragma once
...
struct ReloadState
{
    ...
    // ── Performer-API Result audit (R2): count of != Ok across the whole run ──
