...
// per-build-step seeded perturbation (HARNESS-SPEC §4 Engine B / §6).
// PORT NOTE: identical to _run-reload/spike/bg_compiler.cpp.
...
void backgroundCompileB(ReloadState& st, const char* sineBSource, int32_t sessionIdB)
{
    ...
    if (ok)
    {
        // Handle captured AFTER load, BEFORE link; per-engine handle (§4 / §7.1).
    ...
    else
    {
        ...
        // Do NOT publish bReady on failure: the RT thread detects a missing B at
        // K_XFADE_START and the run FAILS (compile_result != Ok), as intended.
    }
}
