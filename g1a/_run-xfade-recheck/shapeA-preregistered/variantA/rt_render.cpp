    ...
    for (uint32_t k = 0; k < N_BLOCKS; ++k)
    {
        ...
        if (k < K_XFADE_START)
        {
            ...
            while (eventIdx < sched.events.size() && sched.events[eventIdx].block == k)
            {
                ...
                // R1 windowed count: events delivered to A while the B-prepare worker is
                // still in flight (k in [K_PREPARE_START, K_PREPARE_DONE]). PRE-REG §5 R1
                // compares this windowed delivered count to events scheduled in the same
                // window (faithful '==', not the parent's whole-run total).
        ...
        else if (k < K_SWAP)
        {
            ...
            // At k == K_XFADE_START: initial latch (NOT a scheduled event).
            // VARIANT A (PRE-REG §3): do NOT call performers[1].reset() here. The
            // worker created B fresh (phase 0) at K_PREPARE_START and has not advanced
            // it, so B is already in the same fresh-construction state that the xfade
            // oracle renderRefBxfade models. Latch + advance directly.
            if (k == K_XFADE_START)
            {
