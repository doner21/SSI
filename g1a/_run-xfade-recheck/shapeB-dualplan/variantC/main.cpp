...
int main(int argc, char** argv)
{
    ...
    for (uint32_t r = 0; r < R_REPEATS; ++r)
    {
        ...
        // Variant C (diagnostic): create performer B on the MAIN thread before
        // the RT loop, then suppress the worker dispatch. This tests whether
        // thread-of-creation (main vs worker) is the cause of crossfade mismatch.
        st.performers[1] = st.sharedEngine.createPerformer();
        if (!st.performers[1])
        {
            std::cerr << "FATAL: createPerformer(B) on main thread failed (r=" << r << ")\n";
            return 5;
        }
        if (st.performers[1].setBlockSize(BLOCK_SIZE) != cmaj::Result::Ok)
            st.resultViolations.fetch_add(1, std::memory_order_relaxed);
        st.compileOk.store(true, std::memory_order_relaxed);
        st.kPrepareDone = K_PREPARE_START;
        st.bReady.store(true, std::memory_order_release);  // pre-publish: B ready instantly
