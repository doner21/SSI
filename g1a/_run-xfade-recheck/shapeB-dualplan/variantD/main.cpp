...
"Investment-freeze on the live-crossfade event-replay bridge. Fallback strategy: "
"**per-block idempotent frequency re-statement** (re-send current input values every "
"block after swap; state converges within one block period) instead of latch-replay + crossfade.\n";
...
int main(int argc, char** argv)
{
    ...
    std::cout << "=== G1A EVENT-REPLAY spike ("
    ...
    // ===================================================================
    // Variant D: same-thread twin control harness (diagnostic only).
    // Both oracle and live-xfade performers created on MAIN thread (fresh).
    // Oracle:  fresh main + latch stateAt(K_XFADE_START) + in-window events + W_XFADE advance
    // Live:    fresh main + reset() + latch stateAt(K_XFADE_START) + in-window events + W_XFADE advance
    // Gate: maxerr_twin == 0.0 (no warm-up, no cross-thread).
    // ===================================================================
    {
        std::cout << "\n=== Variant D: Same-thread twin control ===\n";

        // --- Oracle path ---
        cmaj::Performer twinOracle = ctx.engine.createPerformer();
        if (!twinOracle) { std::cerr << "FATAL: twinOracle create failed\n"; return 6; }
        twinOracle.setBlockSize(BLOCK_SIZE);
        twinOracle.setInputValue(ctx.freqHandle, (float)sched.stateAt(K_XFADE_START), 0);

        std::vector<float> twinOracleOut(XFADE_SAMPLES, 0.0f);
        uint32_t oracleEventIdx = 0;
        uint32_t oracleEventsDelivered = 0;
        while (oracleEventIdx < sched.events.size() && sched.events[oracleEventIdx].block < K_XFADE_START)
            ++oracleEventIdx;

        for (uint32_t i = 0; i < W_XFADE; ++i)
        {
            uint32_t bk = K_XFADE_START + i;
            while (oracleEventIdx < sched.events.size() && sched.events[oracleEventIdx].block == bk)
            {
                twinOracle.setInputValue(ctx.freqHandle, (float)sched.events[oracleEventIdx].freq, 0);
                ++oracleEventIdx;
                ++oracleEventsDelivered;
            }
            twinOracle.advance();
            twinOracle.copyOutputFrames(ctx.outHandle, twinOracleOut.data() + (uint64_t)i * BLOCK_SIZE, BLOCK_SIZE);
        }

        // --- Live-xfade path ---
        cmaj::Performer twinLive = ctx.engine.createPerformer();
        if (!twinLive) { std::cerr << "FATAL: twinLive create failed\n"; return 6; }
        twinLive.setBlockSize(BLOCK_SIZE);
        twinLive.reset();
        twinLive.setInputValue(ctx.freqHandle, (float)sched.stateAt(K_XFADE_START), 0);

        std::vector<float> twinLiveOut(XFADE_SAMPLES, 0.0f);
        uint32_t liveEventIdx = 0;
        uint32_t liveEventsDelivered = 0;
        while (liveEventIdx < sched.events.size() && sched.events[liveEventIdx].block < K_XFADE_START)
            ++liveEventIdx;

        for (uint32_t i = 0; i < W_XFADE; ++i)
        {
            uint32_t bk = K_XFADE_START + i;
            while (liveEventIdx < sched.events.size() && sched.events[liveEventIdx].block == bk)
            {
                twinLive.setInputValue(ctx.freqHandle, (float)sched.events[liveEventIdx].freq, 0);
                ++liveEventIdx;
                ++liveEventsDelivered;
            }
            twinLive.advance();
            twinLive.copyOutputFrames(ctx.outHandle, twinLiveOut.data() + (uint64_t)i * BLOCK_SIZE, BLOCK_SIZE);
        }

        // --- Metrics ---
        double maxerr_twin = maxAbsDiff(twinOracleOut.data(), twinLiveOut.data(), XFADE_SAMPLES);
        std::cout << "  oracle_events_delivered=" << oracleEventsDelivered
                  << " live_events_delivered=" << liveEventsDelivered
                  << " maxerr_twin=" << maxerr_twin << "\n";

        // Write twin artifacts
        _mkdir("artifacts");
        writeWavFloat(ART_DIR + "TWIN_oracle.wav", twinOracleOut.data(), XFADE_SAMPLES, SAMPLE_RATE);
        writeWavFloat(ART_DIR + "TWIN_live.wav", twinLiveOut.data(), XFADE_SAMPLES, SAMPLE_RATE);

        // metrics.json
        {
            std::ofstream m(ART_DIR + "metrics.json");
            m << "{\n";
            m << "  \"variant\": \"D\",\n";
            m << "  \"description\": \"same-thread twin control — oracle vs live-xfade both on main thread\",\n";
            m << "  \"maxerr_twin\": " << std::setprecision(17) << maxerr_twin << ",\n";
            m << "  \"maxerr_twin_gate\": \"==0.0\",\n";
            m << "  \"twin_pass\": " << (maxerr_twin == 0.0 ? "true" : "false") << ",\n";
            m << "  \"warm_up\": false,\n";
            m << "  \"oracle_events_delivered\": " << oracleEventsDelivered << ",\n";
            m << "  \"live_events_delivered\": " << liveEventsDelivered << ",\n";
            m << "  \"events_scheduled_xfade\": " << sched.eventsInRange(K_XFADE_START, K_SWAP) << "\n";
            m << "}\n";
        }

        // trace.log
        {
            std::ofstream t(ART_DIR + "trace.log");
            t << "Variant D — Same-thread twin control\n";
            t << "========================================\n";
            t << "Both performers created on MAIN thread (fresh).\n";
            t << "Oracle: fresh main + latch stateAt(K_XFADE_START) + in-window events + W_XFADE advance\n";
            t << "Live:   fresh main + reset() + latch stateAt(K_XFADE_START) + in-window events + W_XFADE advance (NO warm-up)\n";
            t << "maxerr_twin = " << maxerr_twin << " (gate: ==0.0)\n";
            t << "twin_pass = " << (maxerr_twin == 0.0 ? "PASS" : "FAIL") << "\n";
        }

        // asan_run.log sentinel
        {
            std::ofstream a(ART_DIR + "asan_run.log");
            a << "ASAN_RUN_CLEAN_EXIT\n";
        }

        std::cout << "\n=== Variant D complete (diagnostic only). ===\n";
        std::cout << "maxerr_twin=" << maxerr_twin
                  << " twin_pass=" << (maxerr_twin == 0.0 ? "PASS" : "FAIL") << "\n";
        return (maxerr_twin == 0.0) ? 0 : 1;
    }

    // ---- unreachable: normal R1-R4 harness not executed in Variant D ----
    ...
    // R4 — determinism + completeness + event audit
    {
        ...
        R.R4_pass = R.repro_ok && (R.blocks_rendered == N_BLOCKS)
                    ...
                    && (R.latch_count_b_xfade == 1)
                    && (R.latch_replay_count == 1);
    }
    ...
    R.R2_pass = (R.asan_violations == 0) && (R.crt_heap_violations == 0)
                ...
                && (R.swap_count == 1) && R.reset_called_on_B
                && R.latch_replay_called && (R.xrun_at_swap == 0)
