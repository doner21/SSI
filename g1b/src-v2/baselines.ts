// src-v2/baselines.ts — G1B-v2 Falsifier: the FIVE baselines (the ladder).
// HARNESS-SPEC-v2.md §9 / PRE-REGISTRATION-v2.md §3.1–§3.6, §5.2.
//
// Carried VERBATIM from v1 (src/baselines.ts) except the single permitted v2 delta:
//   **[v2 CHANGE (1)]** the ablation baseline (v) `factored-ablation` is RENAMED `coupled-ablation`
//   and now reasons on SSI's IDENTICAL **coupled** posterior (`posterior.ts`: `coupledPosterior`,
//   `consequenceConditioned`, `klCoupled`) — with the EIG/VoI machinery still removed. Nothing else
//   changes: baselines (i)–(iv) are byte-for-byte the v1 rules. (PRE-REG-v2 §A, §3.5.)
//
// Each baseline is fit ONLY on the TRAIN split (§5.2 wall) and consumes the SAME per-user fossils
// SSI sees; only the surface/abstain logic differs. All five are deliberately STRONG (not
// strawmen, §3.6) so an SSI win is meaningful and an SSI loss is possible.
//
...
//   (v)   coupled-ablation  — SSI's IDENTICAL COUPLED posterior + surprise threshold, NO EVSI/EIG. §3.5
//
...
//
// ─────────────────────────────────────────────────────────────────────────────
// ABLATION-ISOLATION INVARIANT (§3.5 / §4.5) [GATE] — TIGHTENED in v2
//   `makeCoupledAblation` calls the IDENTICAL `posterior.ts` belief functions SSI uses
//   (`coupledPosterior`, `consequenceConditioned`, `klCoupled` — i.e. SSI's `s1Surprise` IS
//   `klCoupled`) and decides via a surprise threshold tuned on TRAIN ONLY — with NO call into any
//   `evsiAligned`/`irSilentAligned`/`eig` function and NO import from `ssi.ts`. This module imports
//   NOTHING from `src-v2/ssi.ts`; a grep for `evsi`/`eig` here returns nothing. Because the
//   coupling now lives entirely in the shared `posterior.ts` and is given EQUALLY to SSI and (v),
//   the SSI-vs-(v) delta is precisely the clean, load-bearing isolation of the EIG/VoI machinery
//   on a COMMON coupled belief model (PRE-REGISTRATION-v2 §1.3, §3.5, Decision #5).
//
// ─────────────────────────────────────────────────────────────────────────────
...
//
//   B6. [v2] Baseline (v) surprise = klCoupled( P(Intent|C,c) ‖ P(Intent|c) ) over the COUPLED
//       posterior, computed via the SAME posterior.ts functions SSI's S1 screen uses (§4.3) —
//       this is exactly SSI's `s1Surprise` (`// = klCoupled`). It then thresholds that surprise
//       ALONE — no EVSI gate, no EIG ranking, no turn budget. This is SSI minus the EIG/VoI
//       mechanism on a COMMON coupled model, and nothing else (§3.5 isolation).
// ─────────────────────────────────────────────────────────────────────────────
...
// [v2 CHANGE (1)] Baseline (v) uses SSI's IDENTICAL COUPLED belief core — and ONLY this.
// No evsi/eig symbol exists here, and nothing is imported from ssi.ts.
import { consequenceConditioned, coupledPosterior, klCoupled } from "./posterior.js";

// ─────────────────────────────────────────────────────────────────────────────
// Public contract (HARNESS-SPEC-v2 §9)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// (v) [v2 CHANGE (1)] COUPLED model WITHOUT EIG/VoI — the ablation, the load-bearing falsifier (§3.5).
// SSI's IDENTICAL coupled posterior (posterior.ts) + a Bayesian-surprise threshold tuned on TRAIN.
// NO EVSI gate, NO EIG ranking, NO turn budget, NO `EVSI=0 ⇒ abstain`. (NOTES B6)
//
// The ablation's own surprise threshold is tuned by the same TRAIN discipline via this file's own
// single-parameter `tuneThreshold` sweep — it does NOT import or call SSI's `calibrateOnTrain`,
// and references NO EVSI/EIG symbol — so SSI−(v) isolates ONLY the EIG/VoI machinery on a COMMON
// coupled belief model (§3.5/§4.5 [GATE]).
// ─────────────────────────────────────────────────────────────────────────────
class CoupledAblation implements Baseline {
  readonly id = "coupled-ablation" as const;
  private threshold = Number.POSITIVE_INFINITY;

  /**
   * SSI's S1 surprise on the COUPLED posterior:
   *   klCoupled( P(Intent|C,c) ‖ P(Intent|c) ), via posterior.ts ONLY (§4.3).
   * This is literally SSI's `s1Surprise` (`// = klCoupled`); no EVSI/EIG symbol is touched.
   */
  private surprise(cfg: LockedConfig, user: SyntheticUser, ctx: Context, family: FamilyId): number {
    const base = coupledPosterior(cfg, user, ctx);
    const cond = consequenceConditioned(cfg, base, family, ctx);
    return klCoupled(cond, base);
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Factories + the full ladder (HARNESS-SPEC-v2 §9)
// ─────────────────────────────────────────────────────────────────────────────
...
/** §3.5 (v) [v2 CHANGE (1)] COUPLED model WITHOUT EIG/VoI — SSI's coupled posterior + surprise threshold (the falsifier). */
export function makeCoupledAblation(): Baseline {
  return new CoupledAblation();
}
