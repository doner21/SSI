...
import type {
  ...
  ConsequenceCase,
  Context,
  CoupledPosterior, // [v2] irValue now scores against a coupled belief (§4.5; the ONLY signature change here)
  FamilyId,
  Fossil,
  GenreLevel,
  HeldOutContextCombo,
  ...
  LikelihoodSignature,
  LockedConfig,
  PairId, // [v2] used only to marginalize the coupled joints back to per-axis beliefs for the v1 value map
  SubdomainLevel,
  SyntheticUser,
  ThetaState,
} from "./types.js";
...
/**
 * [v2] Reduce a §4.0 `CoupledPosterior` to the per-axis marginal `P(axis = k | ·)` so the v1
 * registered value map is applied UNCHANGED. A paired axis is marginalized from its joint (9
 * row-major cells, `j*3 + l`): the first member sums over its partner's level `l`, the second
 * over `j`. A singleton axis returns its stored per-axis posterior. The marginal of a coupled
 * joint equals the v1 factored belief exactly in the λ_couple → 0 limit, so `Value(IR)` is the
 * v1 map (this module's only v2 delta is the input TYPE, not the value computation).
 */
function axisMarginalFromCoupled(cfg: LockedConfig, belief: CoupledPosterior, axis: AxisId): AxisPosterior {
  for (const pairId of Object.keys(cfg.coupledComponents.pairs) as PairId[]) {
    const [a, b] = cfg.coupledComponents.pairs[pairId];
    if (axis !== a && axis !== b) continue;
    const joint = belief.pairs[pairId];
    const m: [number, number, number] = [0, 0, 0];
    if (axis === a) {
      for (let j = 0; j < 3; j++) for (let l = 0; l < 3; l++) m[j] += joint[j * 3 + l];
    } else {
      for (let j = 0; j < 3; j++) for (let l = 0; l < 3; l++) m[l] += joint[j * 3 + l];
    }
    return m as AxisPosterior;
  }
  // singleton components (NOTES N5): {interrupt} on the flow-extended key c⁺, {bright} on c.
  if (axis === "interrupt") return belief.interrupt;
  if (axis === "bright") return belief.bright;
  throw new Error(`[generative] axis ${axis} is not a member of any coupled component (§4.0)`);
}
 ...
 * [v2] `belief` is now a `CoupledPosterior`; each touched axis is marginalized via
 * `axisMarginalFromCoupled` and scored with the IDENTICAL v1 `matchUtil` value map.
 */
export function irValue(cfg: LockedConfig, ir: IRId, belief: CoupledPosterior, _ctx: Context): number {
  ...
  for (const pair of body.split(";")) {
    ...
    const ordinal = axisSpec(cfg, axis).kind === "ordinal";
    const p = axisMarginalFromCoupled(cfg, belief, axis);
...
function makeCase(
  ...
  return {
    ...
    // [v2 CHANGE (4)] derived, method-INDEPENDENT mandatory-safety scoping flag (§6.1.1): a case is
    // in D_safety iff its family is safety-class. Computed from CONFIG + family ONLY (no method
    // output, no θ_u), so metrics.ts can exclude D_safety IDENTICALLY for SSI and all baselines.
    safetyClass: familySig(cfg, family).familyClass === "safety",
  };
}
