// src-v2/holdout.ts — G1B Falsifier v2 "no-peeking wall" (HARNESS-SPEC-v2 §6 / PRE-REG-v2 §5.1–§5.2).
//
// CARRIED VERBATIM FROM v1 (g1b/src/holdout.ts) — UNCHANGED. None of the four v2 deltas
// (sparse-pairwise coupling, aligned EVSI, TRAIN-only calibration, S6/AQ mandatory-safety split)
// touches the holdout wall: the TRAIN/HELD-OUT partition is registration/integrity machinery only.
// v1 (`g1b/src/`) is the permanent locked record of attempt 1 and is NOT modified by this file.
// The only environmental v2 difference is the new SEED_MASTER (0x5551B_0002n, in src-v2/config.ts),
// which flows in transparently through the injected `holdout-partition` stream — this module's
// code is byte-for-byte the v1 logic.
//
...
//
// Discipline (HARNESS-SPEC-v2 §0): strict TS, ES modules, no `any` in public signatures, NO
...
//   H1. The partition CONTENTS are registered and frozen in PRE-REG-v2 §5.1 / CONFIG
...
//       But HARNESS-SPEC-v2 §6 and PRE-REG-v2 §7.3 require `buildPartition` to be the FIRST
...
//   H2. `tagHeldOut(p, family, ctx)` receives only `family` and `ctx` (HARNESS-SPEC-v2 §6
 ...
 * is drawn (PRE-REG-v2 §7.3 ordering / §5.2 no-peeking wall). The partition contents are
