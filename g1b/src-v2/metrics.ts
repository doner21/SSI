// src-v2/metrics.ts — G1B-v2 Falsifier exact per-user estimators
// (HARNESS-SPEC-v2 §10 / PRE-REGISTRATION-v2 §6 + §6.1.1 + §7.6).
//
...
// calibration/reliability curves (§6.5), the §7.6 negative-control specificity guard, and the
// [v2 CHANGE (4)] separate mandatory-safety (MS) bucket. [ARCH][GATE]
//
// ─────────────────────────────────────────────────────────────────────────────
// [v2 CHANGE (4)] S6/AQ MANDATORY-SAFETY SPLIT (PRE-REG-v2 §6.1.1) — the ONLY metric change.
//
//   The v1 mis-specification: AQ (balanced accuracy of `surface` vs `ws`) charged a *mandated*
//   safety-class surfacing whose discretionary `ws = 0` as an abstention false-positive,
//   penalizing ANY method that obeys the S6 safety override. A mandated surfacing is not a
//   discretionary salience error and must not count against abstention quality.
//
//   The fix (locked, §6.1.1): partition each user's eval cases  D_u = D_u^disc ∪ D_u^safety
//   using ONLY the oracle/signature-defined flag `case.safetyClass` (signature.familyClass ===
//   "safety"). AQ/US/VoI and therefore the §6.4 composite are computed over D_u^disc ONLY; a
//   separate mandatory-safety bucket  MS_u = (Σ_{i∈D_safety} surface_i) / |D_safety|  measures
//   safety compliance. MS is REPORTED, never folded into the composite and never a §9 branch.
//
//   Integrity crux: D_u^safety is method-independent — the SAME cases are excluded for SSI and
//   for all five baselines. No method gets a tailored exclusion (PRE-REG-v2 §6.1.1, §A.4 / §B.6).
//   Everything else in §6 — the AQ/US/VoI/composite/calibration/H-NC estimators and the
//   (w_AQ, w_VoI, w_US) = (0.40, 0.40, 0.20) weights — is CARRIED VERBATIM from v1.
// ─────────────────────────────────────────────────────────────────────────────
//
...
//
// Locked conventions honored (PRE-REGISTRATION-v2 §6 [GATE]):
...
//   • [v2] MS = mandatory-safety compliance over D_safety, REPORTED, never gated (§6.1.1).
//
// ─────────────────────────────────────────────────────────────────────────────
// NOTES (spec-ambiguity interpretations — no new parameters; calibrationBins is the only
//        knob and it comes from CONFIG):
//
//   M1. Per-user scope. The §6.1–§6.3 estimators are defined PER USER (over the per-user case
//       set D_u). The caller passes ONE user's eval cases + that user's decisions; the
//       implementation computes over whatever (decisions, cases) pair it is given, so per-user
//       slicing by the caller yields the per-user statistic. No new parameter is introduced.
//       [v2] The AQ/US/VoI/calibration/H-NC functions take the user's DISCRETIONARY cases
//       (D_disc); `perUserMetrics` performs the §6.1.1 partition internally and routes D_disc to
//       those estimators and D_safety to `mandatorySafety`.
//
//   M2. Decision↔case join. Decisions are matched to cases by `caseId` (the only shared key,
//       §2). A case with no matching decision is treated as an ABSTAIN (surface = 0, score = 0):
//       a method that never decided a case did not surface it. Under the locked run every eval
//       case receives exactly one decision, so this default is unreachable in practice; it only
//       guards against a malformed decision stream.
//
//   M3. Empty-class half-term (§6.1). Balanced accuracy needs both a positive (ws=1) and a
//       negative (ws=0) case for the user. PRE-REGISTRATION-v2 §5.3 GUARANTEES both classes per
//       user, so neither denominator is empty under the locked run. For the residual/defensive
//       empty-class path §6.1 says the missing half-term is "set to the registered prior rate":
//       we use the chance rate 0.5 — the prior/expected value of a balanced-accuracy half-term
//       for a non-informative decision, which keeps AQ ∈ [0,1] and introduces NO new numeric
//       parameter. (Unreachable under §5.3.)
//
...
//
//   M6. [v2] Mandatory-safety empty set (§6.1.1). If a user has no safety-class case, MS returns
//       1 (vacuously compliant — there is no mandated surfacing owed). §5.3 guarantees a small
//       registered safety fraction per user, so this is defensive only; MS is reported, never
//       gated, so this default cannot influence any verdict.
...
// ─────────────────────────────────────────────────────────────────────────────
// [v2 CHANGE (4)] §6.1.1 — the oracle/signature-defined, method-independent partition.
// safety = cases with `case.safetyClass === true` (signature.familyClass === "safety");
// the SAME D_safety is excluded for SSI and for ALL five baselines (integrity crux). [ARCH][GATE]
// ─────────────────────────────────────────────────────────────────────────────

export function partitionCases(cases: readonly ConsequenceCase[]): {
  disc: readonly ConsequenceCase[];
  safety: readonly ConsequenceCase[];
} {
  const disc: ConsequenceCase[] = [];
  const safety: ConsequenceCase[] = [];
  for (const c of cases) {
    if (c.safetyClass) safety.push(c);
    else disc.push(c);
  }
  return { disc, safety };
}
...
// [v2] Computed over the user's DISCRETIONARY cases (D_disc) ONLY (§6.1.1).
// ─────────────────────────────────────────────────────────────────────────────
...
// [v2] Composite is over D_disc only (its inputs aq/us/voi are computed over D_disc); weights
// are UNCHANGED from v1 (PRE-REG-v2 §6.1.1: MS is reported, not folded into the composite).
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// [v2 CHANGE (4)] §6.1.1 — mandatory-safety compliance MS over D_safety (REPORTED, NOT gated).
// MS_u(M) = (Σ_{i ∈ D_safety} surface_M(i)) / |D_safety| ∈ [0,1]. Method-independent set, so the
// SAME D_safety is scored for every method; a method that never surfaces scores MS = 0, a method
// that obeys S6 scores MS = 1 (PRE-REG-v2 §6.1.1). [ARCH][GATE]
// ─────────────────────────────────────────────────────────────────────────────

export function mandatorySafety(
  decisions: readonly MethodDecision[],
  safety: readonly ConsequenceCase[],
): number {
  const j = join(decisions, safety);
  if (j.length === 0) return 1; // vacuously compliant — no mandated surfacing owed (NOTES M6)
  let surfaced = 0;
  for (const x of j) if (x.surface === 1) surfaced++;
  return surfaced / j.length;
}

// ─────────────────────────────────────────────────────────────────────────────
// §6.4 / §6.1.1 — assemble PerUserMetrics: aq/us/voi/composite over D_disc + ms over D_safety.
// The §6.1.1 partition is performed HERE (method-independently) so every method's per-user
// composite excludes the SAME safety-class cases. [ARCH][GATE]
// ─────────────────────────────────────────────────────────────────────────────
...
export function perUserMetrics(
  ...
  const { disc, safety } = partitionCases(cases); // [v2] §6.1.1 method-independent split
  const aq = abstentionQuality(decisions, disc); // over D_disc only
  const us = usefulness(decisions, disc); // over D_disc only
  const voi = answerChangingVoI(decisions, disc); // over D_disc only
  const ms = mandatorySafety(decisions, safety); // over D_safety (reported, not gated)
  return { aq, us, voi, composite: composite(cfg, { aq, us, voi }), ms };
}

// ─────────────────────────────────────────────────────────────────────────────
// §6.5 — Calibration / reliability curves + ECE (DIAGNOSTIC, NEVER GATED). [ARCH]
// [v2] Computed over D_disc (the caller passes the discretionary slice; §10).
// ─────────────────────────────────────────────────────────────────────────────
...
// [v2] `disc` = the user's discretionary cases (D_disc).
...
// [v2] Over D_disc — negative controls are discretionary by construction (§10), so the §6.1.1
// safety exclusion never removes a negative-control case.
// ─────────────────────────────────────────────────────────────────────────────
